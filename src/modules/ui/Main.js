...
export default class Main extends Phaser.GameObjects.Group {
  ...
  constructor(scene) {
    ...
    Object.keys(jsonData).forEach((key) => {
      ...
      window[heldName] = scene.add.text(((config.scale.width * 8) / 128), ((config.scale.height * heldCoord) / 128), `${key}: ${game.inventory.convert(`${game.inventory[key]}`)}`).setColor('#000000').setInteractive().setFontSize(28).setFontFamily('"DejaVu Sans Mono"').setDepth(1);
      heightCoord += 12;
      textHeightCoord += 12;
      heldCoord += 5;
    });
